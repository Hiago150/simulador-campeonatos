...
const SilkPlane = forwardRef<Mesh, { uniforms: SilkUniforms }>(function SilkPlane({ uniforms }, ref) {
  ...
  useFrame((_, delta) => {
    const mesh = (ref as React.RefObject<Mesh>).current
    if (mesh) (mesh.material as ShaderMaterial).uniforms.uTime.value += 0.1 * delta
  })
...
const Silk = ({ speed = 5, scale = 1, color = '#7B7481', noiseIntensity = 1.5, rotation = 0 }: SilkProps): JSX.Element => {
